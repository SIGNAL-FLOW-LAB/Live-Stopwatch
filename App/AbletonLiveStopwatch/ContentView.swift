...
struct ContentView: View {
    ...
    private func timerDisplay(
        metrics: LayoutMetrics
    ) -> some View {
        Text(stopwatch.formattedTime)
            .font(
                SFFont.timer(metrics.timerFontSize)
            )
    ...
    private func informationBlock(
        ...
        VStack(
            alignment: .leading,
            spacing: metrics.labelSpacing
        ) {
            Text(label)
                .font(
                    SFFont.caption(metrics.labelFontSize)
                )
                .foregroundColor(
                    SFTheme.white.opacity(0.48)
                )
                .lineLimit(1)
        ...
        .padding(
            .vertical,
            metrics.informationVerticalPadding
        )
    }
    ...
    private func statusBar(
        ...
        .padding(
            .horizontal,
            metrics.statusHorizontalPadding
        )
    ...
    private func updateWindow(
        ...
        guard !didConfigureWindow else {
            return
        }

        didConfigureWindow = true
        MainWindowFrameStore.shared.attach(
            to: window
        )
    }
...
private struct LayoutMetrics {
    ...
    init(size: CGSize) {
        ...
        informationVerticalPadding = min(
            max(10.0 * scale, 6.0),
            26.0
        )
        ...
        primaryTitleFontSize = min(
            max(30.0 * scale, 17.0),
            78.0
        )

        secondaryTitleFontSize = min(
            max(24.0 * scale, 14.0),
            64.0
        )
        ...
        statusHorizontalPadding = compact
            ? 10.0
            : horizontalPadding

        statusBarHeight = compact
            ? 38.0
            : min(
                max(62.0 * scale, 58.0),
                92.0
        ...
        configuration.label
            ...
            .background(
                configuration.isPressed
                    ? SFTheme.secondary
                    : SFTheme.white.opacity(0.14)
            )
            .cornerRadius(7)
    }
}

// MARK: - Main Window Frame Store

private final class MainWindowFrameStore {
    static let shared =
        MainWindowFrameStore()

    private let defaults =
        UserDefaults.standard

    private let frameKey =
        "LiveStopwatch.MainWindowFrame.v2"
    ...
    private func isVisibleOnAnyScreen(
        _ frame: NSRect
    ) -> Bool {
